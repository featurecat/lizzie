...
public class VariationTree {
    ...
    public void drawTree(Graphics2D g, int posx, int posy, int startLane, int maxposy, BoardHistoryNode startNode,
            int variationNumber, boolean isMain) {
        if (isMain)
        ...
        if (lane > 0) {
            if (lane - startLane > 0 || variationNumber > 1) {
                ...
                g.drawLine(curposx + dotoffset, posy + dotoffset, curposx + dotoffset - XSPACING,
                        posy + dotoffset - YSPACING);
                g.drawLine(posx + (startLane - variationNumber) * XSPACING + 2 * dotoffset, posy - YSPACING + dotoffset,
                        curposx + dotoffset - XSPACING, posy + dotoffset - YSPACING);
            } else {
                // Just an angled line
                g.drawLine(curposx + dotoffset, posy + dotoffset, curposx + 2 * dotoffset - XSPACING,
    ...
    public BoardHistoryNode inSubtree(int posx, int posy, int mouseX, int mouseY, int startLane, int maxposy,
            BoardHistoryNode startNode, int variationNumber) {
        // Finds depth on leftmost variation of this tree
        int depth = BoardHistoryList.getDepth(startNode) + 1;
        int lane = startLane;
        // Figures out how far out too the right (which lane) we have to go not to
        ...
        while (cur.next() != null && posy + YSPACING < maxposy) {
            ...
            if (Math.abs(mouseX - curposx - dotoffset) < XSPACING / 2
                    && Math.abs(mouseY - posy - dotoffset) < YSPACING / 2)
                return cur;
    ...
    public int jumpVariationTree(int mouseX, int mouseY) {
        ...
        if (node == null)
            return -1; // check if any error occur
